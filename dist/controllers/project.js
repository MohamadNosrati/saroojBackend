...
export const getAllProjects = catchAsync(async (req, res) => {
    const query = new ApiFeatures(ProjectModel.find(), req?.query);
    const projects = await query.filtering().sorting().pagination()
        .populate("pictureId", ["image", "id"])
        .populate("categoryId", ["title", "id"]).execute();
    const totalCount = await new ApiFeatures(ProjectModel.find(), req?.query).filtering().getTotalCount();
...
export const findProjectBySlug = catchAsync(async (req, res, next) => {
    const project = await ProjectModel.findOne({ title: decodeURIComponent(req.params?.slug?.trim()) });
    if (!project) {
...
export const deleteProject = catchAsync(async (req, res, next) => {
    ...
    const imageIds = project?.images?.map(({ before, after }) => [before?.pictureId?.id, after?.pictureId?.id])?.flatMap(elem => elem);
    const imageNames = project?.images?.map(({ before, after }) => [before?.pictureId?.image, after?.pictureId?.image])?.flatMap(elem => elem);
...
export const updateProject = catchAsync(async (req, res, next) => {
    ...
    const newImageIds = newProject?.images?.map(({ before, after }) => [before?.pictureId?.id, after?.pictureId?.id])?.flatMap((elem) => elem?.toString());
    const prvImageIds = project?.images?.map(({ before, after }) => [before?.pictureId?.id, after?.pictureId?.id])?.flatMap((elem) => elem?.toString());
    ;
    const removedIds = prvImageIds?.filter((item) => !newImageIds.includes(item));
    console.log("removedIds", removedIds);
    await PictureModel.deleteMany({ _id: { $in: removedIds } });
    const newImageNames = newProject?.images?.map(({ before, after }) => [before?.pictureId?.image, after?.pictureId?.image])?.flatMap((elem) => elem);
    ;
    const prvImageNames = project?.images?.map(({ before, after }) => [before?.pictureId?.image, after?.pictureId?.image])?.flatMap((elem) => elem);
    ;
    const removedImageNames = prvImageNames?.filter((item) => !newImageNames.includes(item));
    removedImageNames?.forEach((item) => unlinkFile(item));
    res.status(201).json({
        status: 201,
