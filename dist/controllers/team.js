...
export const createTeamate = catchAsync(async (req, res) => {
...
export const getAllTeamates = catchAsync(async (req, res) => {
...
export const findTeamate = catchAsync(async (req, res, next) => {
    const teamate = await checkExists(TeamModel, next, "عضو تیم", req.params?.id, "pictureId", ["image", "id"]);
    res.status(200).json({
        status: 200,
...
export const deleteTeamate = catchAsync(async (req, res, next) => {
...
export const updateTeamate = catchAsync(async (req, res, next) => {
    const teamate = await checkExists(TeamModel, next, "دسته بندی", req.params?.id);
    const newTeamte = await TeamModel.findByIdAndUpdate(req.params?.id, {
        $set: req.body,
    }, {
        new: true,
        runValidators: true,
